---
title: "初心者セッション"
subtitle: "R入門〜データハンドリング入門"
author: "y__mattu"
date: "2024-10-19 TokyoR #115"
output:
...
- 2024/10/19時点の最新版は、4.4.1
    - 本日の内容に関しては、4.X.X以上であれば概ね問題ないと思われます
    
![](./slide_img/r441_2.png)
...
1. tidy data をざっくり理解する
2. dplyrで簡単な集計ができるようになる

# 1. tidy data {#tidydata}

## 使用するパッケージ {#package_tidyrr}
<div class="column1">
...
## データの形式1. Wide 型{#wide_data}
２つのデータ形式(例: カテゴリごとの購買金額(千円))
```{r echo=FALSE}
dat_messy <- tibble::tibble(user = c('A', 'B', 'C'),
                    category1 = c(10, 15, 8),
                    category2 = c(2, 4, 5))
dat_messy %>%
...
## データの形式2. Long 型{#long_data}
２つのデータ形式(例: カテゴリごとの購買金額(千円))
```{r echo=FALSE}
dat_tidy <- dat_messy %>%
  tidyr::pivot_longer(names_to = "category", values_to = "price", -user)
dat_tidy %>%
  DT::datatable(extensions = 'FixedColumns',
  options = list(
    dom = 't',
